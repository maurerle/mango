    ...
    def setup(self):
        ...
        def list_condition(content, meta):
            return isinstance(content, list) and all(
                [type(e) == SimpleBid for e in content]
            )
        ...
        self.context.subscribe_message(self, self.handle_list_message, list_condition)
...
async def main(start: datetime):
    ...
    for j in range(5):
        ...
        for i in range(demand_count):
            ...
            c = containers[i % len(addr)]
            agent = BiddingAgent(
                c, ad, inter_market.aid, volume=-80, price=0.03 + 0.05 * (i % 9)
            )
            agents.append(agent)
            receiver_ids.append((ad, agent.aid))

        inter_market.add_role(BiddingRole(market.context.addr, market.aid, 0, 0))
        inter_market.add_role(
            TwoSidedMarketRole(start - timedelta(seconds=60), receiver_ids)
        )
